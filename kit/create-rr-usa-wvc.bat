...
set DEST_ID=RMCETO
set DEST_NAME=Mario Kart Retro Rewind WiiVC
...
if exist workdir.tmp (echo Temporary workdir already exists. Stop. & exit /b 1)
if exist new-image-wvc (echo WiiVC output directory already exists. Stop. & exit /b 1)
...
wit -E$ copy workdir.tmp -T0 --DEST new-image-wvc/$X -ovv --links --id "%DEST_ID%" --ticket-id=RMCR --tmd-id=RMCR --boot-id=RMC%REGION% --security-fix --name "%DEST_NAME%" --%IMAGE_TYPE% %SPLT%
rem Leave temporary workdir in place for inspection
rem Leave generated config in place for inspection
