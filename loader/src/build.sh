...
# The linker script's MEMORY regions fail the link if either part outgrows its slot.
...
wc -c build/bootstrap-main.bin build/bootstrap-low.bin

# The Windows build script consumes these prebuilt copies.
cp build/bootstrap-main.bin build/bootstrap-low.bin ../prebuilt/
